		...
		draggable({
			...
			onGenerateDragPreview({ nativeSetDragImage }) {
				setCustomNativeDragPreview({
					...
					render: ({ container }) => {
						return render(h("div", { class: "bg-white text-muted-foreground rounded-md text-sm font-medium px-3 py-1.5 border" }, node.value.name || item.id), container);
					},
					nativeSetDragImage,
				});
			},
		...
		<div class="flex items-center gap-2 w-full">
			...
			<div class="flex items-center gap-1 opacity-0 group-hover:opacity-100">
				<TooltipProvider>
					...
					<Tooltip>
						<TooltipTrigger as-child>
							<Button size="icon" variant="ghost" class="h-6 w-6 text-destructive hover:text-destructive" @click.stop="handleDelete">
